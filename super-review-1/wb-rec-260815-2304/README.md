# workbooks-recorder bundle

This is a session recording captured by workbooks-recorder. Intended consumers: LLMs turning it into a runbook, Playwright for replay, or a human double-clicking `index.html` to scrub through the flow.
...
- **Playwright**: `replay.spec.ts` is a runnable mechanical export — copy it into a Playwright project and `npx playwright test`. Masked fields resolve via `process.env.WB_SECRET_*`.
...
### `PROMPT.md`
A ready-to-paste prompt template for converting this bundle to a `wb` workbook via an LLM. See the "Converting to a `wb` workbook" section below.
...
- Element carries the `.wb-mask` class
...
- `wb-mask`, `data-wb-mask`, `data-sensitive` (this extension's own)
...
This is intentional: the recorder emits **semantic events**, not a replay trace. If you need a replay-grade capture, use rrweb or Playwright trace; this format optimizes for human/LLM comprehension and conversion to runnable workbooks.

## Converting to a `wb` workbook
...
`/Users/jmitch/dev/workbooks-dev/workbooks/examples/browser-demo.md`

## Schema version

The current schema version is declared in `manifest.format` as `workbooks-recording/2.2`.
