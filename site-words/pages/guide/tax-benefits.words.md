---
id: guide-tax-benefits
title_for_humans: "Guide — Tax benefits"
live_path: "pages/tax-benefits.html"
live_url: "/pages/tax-benefits.html"
body_master_today: "content/guide/tax-benefits.body.html"
layout_today: "templates/layouts/guide-tax-benefits.html"
coverage_status: complete
inventoried_at: "2026-08-26"
human_howto: "Edit only visible wording under each ## heading. Leave {#ids} alone. Ignore *.assistive.words.md files."
omitted_by_policy:
  - "Shared nav/footer → common/chrome.words.md"
  - "Data-file numeric/product values"
  - "JS-only Explore runtime strings still in src/ until generated pack"
  - "Screen-reader / frame-title spoken names → sibling *.assistive.words.md (do not edit for marketing)"
---

# Guide — Tax benefits

Edit the **visible text under each heading**. Leave the `{#…}` code alone.

You only need this file for wording people **see**. Spoken/screen-reader names live in a separate file you can ignore.

Use **Jump to** below to open any line in this file.

## Jump to

- [Link — Overview](#t.overview.5)
- [Link — Documents](#t.documents.6)
- [Link — Tax benefits](#t.tax_benefits.7)
- [Link — Concessions](#t.concessions.8)
- [Link — Insurance](#t.insurance.9)
- [Link — If something goes wrong](#t.if_something_goes_wrong.10)
- [Link — Explore banks](#t.explore_banks.11)
- [Heading — Know how much of this loan can still reduce your tax this year.](#t.know_how_much_of_this_loan_can_still_reduce_your.12)
- [Text — Contents](#t.contents.13)
- [Text — Interest payment](#t.interest_payment.14)
- [Text — Principal repayment](#t.principal_repayment.15)
- [Text — Under construction](#t.under_construction.16)
- [Text — Joint loan](#t.joint_loan.17)
- [Text — How to claim](#t.how_to_claim.18)
- [Text — Other](#t.other.19)
- [Text — Can you claim the interest on tax return?](#t.can_you_claim_the_interest_on_tax_return.20)
- [Text — (opens official Income Tax page)](#t.opens_official_income_tax_page.21)
- [Button — You live in it](#t.you_live_in_it.22)
- [Button — You rent it out](#t.you_rent_it_out.23)
- [Text — Old tax regime](#t.old_tax_regime.24)
- [Text — Up to](#t.up_to.25)
- [Text — ₹2 lakh](#t.2_lakh.26)
- [Text — a year](#t.a_year.27)
- [Text — New tax regime](#t.new_tax_regime.28)
- [Text — &nbsp;](#t.nbsp.29)
- [Text — Cannot be claimed](#t.cannot_be_claimed.30)
- [Text — What changes this limit](#t.what_changes_this_limit.31)
- [Text — ₹2 lakh limit](#t.2_lakh_limit.32)
- [Text — Needs a loan to buy or build, and the home ready within 5 years.](#t.needs_a_loan_to_buy_or_build_and_the_home_ready_.33)
- [Text — Loan taken for repair, renewal or reconstruction](#t.loan_taken_for_repair_renewal_or_reconstruction.34)
- [Text — Only ₹30,000 a year.](#t.only_30_000_a_year.35)
- [Text — No fixed upper cap](#t.no_fixed_upper_cap.36)
- [Text — Can be claimed against rental income](#t.can_be_claimed_against_rental_income.37)
- [Text — The 5-year ready rule does not apply when you rent it out.](#t.the_5_year_ready_rule_does_not_apply_when_you_re.38)
- [Text — Up to two homes can be treated as self-occupied.](#t.up_to_two_homes_can_be_treated_as_self_occupied.39)
- [Text — Can you claim the principal on tax return?](#t.can_you_claim_the_principal_on_tax_return.40)
- [Text — ₹1.5 lakh](#t.1_5_lakh.41)
- [Text — a year under the old tax regime](#t.a_year_under_the_old_tax_regime.42)
- [Text — The principal repayment limit under Section 80C is shared with other …](#t.the_principal_repayment_limit_under_section_80c_.43)
- [Text — You can only claim tax benefits on principal repayment starting from …](#t.you_can_only_claim_tax_benefits_on_principal_rep.44)
- [Text — If you sell the home within 5 years of getting possession, all previo…](#t.if_you_sell_the_home_within_5_years_of_getting_p.45)
- [Text — What if the home is under construction?](#t.what_if_the_home_is_under_construction.46)
- [Text — Claim the interest starting from the year you get the keys.](#t.claim_the_interest_starting_from_the_year_you_ge.47)
- [Text — Pre-construction interest accrued prior to taking possession can be c…](#t.pre_construction_interest_accrued_prior_to_takin.48)
- [Text — Post-possession interest paid during the year can be claimed in full …](#t.post_possession_interest_paid_during_the_year_ca.49)
- [Text — About the ₹2 lakh interest limit](#t.about_the_2_lakh_interest_limit.50)
- [Text — Under the old tax regime, pre-possession interest and post-possession i…](#t.under_the_old_tax_regime_pre_possession_and_afte.52)
- [Text — Can both co-owners claim tax benefits?](#t.can_both_co_owners_claim_tax_benefits.54)
- [Text — Yes, when both names are on the property papers and the loan, and eac…](#t.yes_when_both_names_are_on_the_property_papers_a.55)
- [Text — How do you claim it?](#t.how_do_you_claim_it.56)
- [Text — Interest and principal go in different sections of your tax return.](#t.interest_and_principal_go_in_different_sections_.57)
- [Text — Claimed under](#t.claimed_under.58)
- [Text — Claimed as](#t.claimed_as.59)
- [Text — If salaried, share the interest and principal figures with your emplo…](#t.if_salaried_share_the_interest_and_principal_fig.60)
- [Text — Documents you need](#t.documents_you_need.61)
- [Text — Interest certificate](#t.interest_certificate.62)
- [Text — From the lender for the year. Shows interest payment and principal re…](#t.from_the_lender_for_the_year_shows_interest_paym.63)
- [Text — Possession proof](#t.possession_proof.64)
- [Text — Needed when principal repayment and pre-possession interest payment s…](#t.needed_when_principal_repayment_and_pre_possessi.65)
- [Text — Anything else on tax?](#t.anything_else_on_tax.66)
- [Text — There are a few extra benefits beyond the main interest and principal…](#t.there_are_a_few_extra_benefits_beyond_the_main_i.67)
- [Text — Claimed in the year you pay them, within the same ₹1.5 lakh principal…](#t.claimed_in_the_year_you_pay_them_within_the_same.68)
- [Text — Premium can sit in the same ₹1.5 lakh basket if you pay it yourself —…](#t.premium_can_sit_in_the_same_1_5_lakh_basket_if_y.69)
- [Text — If interest creates a loss, only up to ₹2 lakh can be set off against…](#t.if_interest_creates_a_loss_only_up_to_2_lakh_can.70)
- [Text — You usually claim principal repayment on tax only after a house is bu…](#t.you_usually_claim_principal_repayment_on_tax_onl.72)
- [Text — Under the old tax regime only, and only if this was your first home, …](#t.under_the_old_tax_regime_only_and_only_if_this_w.73)
- [text](#t.your_home_loan_emi.75)
- [text](#t.has_tax_benefits.76)
- [text](#t.yes_your_limit_depends_on_who_lives_in_the_home_.77)
- [text](#t.section_22.78)
- [text](#t.nbsp_ita_2025.79)
- [text](#t.if_either_is_missing_only_30_000_a_year.80)
- [text](#t.yes_only_under_the_old_tax_regime.81)
- [text](#t.schedule_xv.82)
- [text](#t.income_from_house_property.83)
- [text](#t.a_deduction_from_total_income.84)
- [text](#t.see_tax_claims_calculator.85)
- [text](#t.stamp_duty_and_registration_property_purchase_on.86)
- [text](#t.home_loan_protection_insurance.88)
- [text](#t.house_property_loss_old_tax_regime.89)
- [text](#t.house_property_loss_new_tax_regime.91)
- [text](#t.interest_on_a_rented_home_can_still_reduce_renta.92)
- [text](#t.loan_for_a_plot_only_no_house_yet.93)
- [text](#t.older_first_home_extras_only_if_your_loan_year_m.94)
- [text](#t.section_130.95)
- [text](#t.covered_sanctions_from_1_april_2016_to_31_march_.96)
- [text](#t.section_131.97)
- [text](#t.covered_sanctions_from_1_april_2019_to_31_march_.98)

<a id="t.overview.5"></a>

## Link — Overview {#t.overview.5}
Overview

<a id="t.documents.6"></a>

## Link — Documents {#t.documents.6}
Documents

<a id="t.tax_benefits.7"></a>

## Link — Tax benefits {#t.tax_benefits.7}
Tax benefits

<a id="t.concessions.8"></a>

## Link — Concessions {#t.concessions.8}
Concessions

<a id="t.insurance.9"></a>

## Link — Insurance {#t.insurance.9}
Insurance

<a id="t.if_something_goes_wrong.10"></a>

## Link — If something goes wrong {#t.if_something_goes_wrong.10}
If something goes wrong

<a id="t.explore_banks.11"></a>

## Link — Explore banks {#t.explore_banks.11}
Explore banks

<a id="t.know_how_much_of_this_loan_can_still_reduce_your.12"></a>

## Heading — Know how much of this loan can still reduce your tax this year. {#t.know_how_much_of_this_loan_can_still_reduce_your.12}
Know how much of this loan can still reduce your tax this year.

<a id="t.contents.13"></a>

## Text — Contents {#t.contents.13}
Contents

<a id="t.interest_payment.14"></a>

## Text — Interest payment {#t.interest_payment.14}
Interest payment

<a id="t.principal_repayment.15"></a>

## Text — Principal repayment {#t.principal_repayment.15}
Principal repayment

<a id="t.under_construction.16"></a>

## Text — Under construction {#t.under_construction.16}
Under construction

<a id="t.joint_loan.17"></a>

## Text — Joint loan {#t.joint_loan.17}
Joint loan

<a id="t.how_to_claim.18"></a>

## Text — How to claim {#t.how_to_claim.18}
How to claim

<a id="t.other.19"></a>

## Text — Other {#t.other.19}
Other

<a id="t.can_you_claim_the_interest_on_tax_return.20"></a>

## Text — Can you claim the interest on tax return? {#t.can_you_claim_the_interest_on_tax_return.20}
Can you claim interest in your ITR?

<a id="t.opens_official_income_tax_page.21"></a>

## Text — (opens official Income Tax page) {#t.opens_official_income_tax_page.21}
(opens official Income Tax page)

<a id="t.you_live_in_it.22"></a>

## Button — You live in it {#t.you_live_in_it.22}
You live in it

<a id="t.you_rent_it_out.23"></a>

## Button — You rent it out {#t.you_rent_it_out.23}
You rent it out

<a id="t.old_tax_regime.24"></a>

## Text — Old tax regime {#t.old_tax_regime.24}
Old tax regime

<a id="t.up_to.25"></a>

## Text — Up to {#t.up_to.25}
Up to

<a id="t.2_lakh.26"></a>

## Text — ₹2 lakh {#t.2_lakh.26}
₹2 lakh

<a id="t.a_year.27"></a>

## Text — a year {#t.a_year.27}
a year

<a id="t.new_tax_regime.28"></a>

## Text — New tax regime {#t.new_tax_regime.28}
New tax regime

<a id="t.nbsp.29"></a>

## Text — &nbsp; {#t.nbsp.29}
&nbsp;

<a id="t.cannot_be_claimed.30"></a>

## Text — Cannot be claimed {#t.cannot_be_claimed.30}
Cannot be claimed

<a id="t.what_changes_this_limit.31"></a>

## Text — What changes this limit {#t.what_changes_this_limit.31}
What changes this limit

<a id="t.2_lakh_limit.32"></a>

## Text — ₹2 lakh limit {#t.2_lakh_limit.32}
₹2 lakh limit

<a id="t.needs_a_loan_to_buy_or_build_and_the_home_ready_.33"></a>

## Text — Needs a loan to buy or build, and the home ready within 5 years. {#t.needs_a_loan_to_buy_or_build_and_the_home_ready_.33}
Needs a loan to buy or build, and the home ready within 5 years.

<a id="t.loan_taken_for_repair_renewal_or_reconstruction.34"></a>

## Text — Loan taken for repair, renewal or reconstruction {#t.loan_taken_for_repair_renewal_or_reconstruction.34}
Loan taken for repair, renewal or reconstruction

<a id="t.only_30_000_a_year.35"></a>

## Text — Only ₹30,000 a year. {#t.only_30_000_a_year.35}
Only ₹30,000 a year.

<a id="t.no_fixed_upper_cap.36"></a>

## Text — No fixed upper cap {#t.no_fixed_upper_cap.36}
No fixed upper cap

<a id="t.can_be_claimed_against_rental_income.37"></a>

## Text — Can be claimed against rental income {#t.can_be_claimed_against_rental_income.37}
Can be claimed against rental income

<a id="t.the_5_year_ready_rule_does_not_apply_when_you_re.38"></a>

## Text — The 5-year ready rule does not apply when you rent it out. {#t.the_5_year_ready_rule_does_not_apply_when_you_re.38}
The 5-year ready rule does not apply when you rent it out.

<a id="t.up_to_two_homes_can_be_treated_as_self_occupied.39"></a>

## Text — Up to two homes can be treated as self-occupied. {#t.up_to_two_homes_can_be_treated_as_self_occupied.39}
Up to two homes can be treated as self-occupied.

<a id="t.can_you_claim_the_principal_on_tax_return.40"></a>

## Text — Can you claim the principal on tax return? {#t.can_you_claim_the_principal_on_tax_return.40}
Can you claim prinicpal in your ITR?

<a id="t.1_5_lakh.41"></a>

## Text — ₹1.5 lakh {#t.1_5_lakh.41}
₹1.5 lakh

<a id="t.a_year_under_the_old_tax_regime.42"></a>

## Text — a year under the old tax regime {#t.a_year_under_the_old_tax_regime.42}
a year under the old tax regime

<a id="t.the_principal_repayment_limit_under_section_80c_.43"></a>

## Text — The principal repayment limit under Section 80C is shared with other … {#t.the_principal_repayment_limit_under_section_80c_.43}
This principal limit is counted together with EPF, life insurance, and PPF as per section 80C

<a id="t.you_can_only_claim_tax_benefits_on_principal_rep.44"></a>

## Text — You can only claim tax benefits on principal repayment starting from … {#t.you_can_only_claim_tax_benefits_on_principal_rep.44}
Principal repayment deductions start only after construction is finished and possession is taken.

<a id="t.if_you_sell_the_home_within_5_years_of_getting_p.45"></a>

## Text — If you sell the home within 5 years of getting possession, all previo… {#t.if_you_sell_the_home_within_5_years_of_getting_p.45}
If the property is sold within 5 years, past principal tax savings must be paid back, but interest tax savings are kept.

<a id="t.what_if_the_home_is_under_construction.46"></a>

## Text — What if the home is under construction? {#t.what_if_the_home_is_under_construction.46}
What if the home is under construction?

<a id="t.claim_the_interest_starting_from_the_year_you_ge.47"></a>

## Text — Claim the interest starting from the year you get the keys. {#t.claim_the_interest_starting_from_the_year_you_ge.47}
Claim the interest starting from the year you get the possession.

<a id="t.pre_construction_interest_accrued_prior_to_takin.48"></a>

## Text — Pre-construction interest accrued prior to taking possession can be c… {#t.pre_construction_interest_accrued_prior_to_takin.48}
Interest built up before getting possession is claimed in five equal parts starting from the possession year.

<a id="t.post_possession_interest_paid_during_the_year_ca.49"></a>

## Text — Post-possession interest paid during the year can be claimed in full … {#t.post_possession_interest_paid_during_the_year_ca.49}
Interest paid each year is claimed in full under Section 24(b).

<a id="t.about_the_2_lakh_interest_limit.50"></a>

## Text — About the ₹2 lakh interest limit {#t.about_the_2_lakh_interest_limit.50}
About the ₹2 lakh interest limit

<a id="t.under_the_old_tax_regime_pre_possession_and_afte.52"></a>

## Text — Under the old tax regime, pre-possession interest and post-possession i… {#t.under_the_old_tax_regime_pre_possession_and_afte.52}
Under the old tax regime, pre-possession interest and post-possession interest together stay within the ₹2 lakh limit.

<a id="t.can_both_co_owners_claim_tax_benefits.54"></a>

## Text — Can both co-owners claim tax benefits? {#t.can_both_co_owners_claim_tax_benefits.54}
Can both co-owners claim tax benefits?

<a id="t.yes_when_both_names_are_on_the_property_papers_a.55"></a>

## Text — Yes, when both names are on the property papers and the loan, and eac… {#t.yes_when_both_names_are_on_the_property_papers_a.55}
Yes, both co-owners can claim tax deductions if their names are on the papers and the loan, and each pays their share based on their ownership percentage and the amount they actually paid.

<a id="t.how_do_you_claim_it.56"></a>

## Text — How do you claim it? {#t.how_do_you_claim_it.56}
How do you claim it?

<a id="t.interest_and_principal_go_in_different_sections_.57"></a>

## Text — Interest and principal go in different sections of your tax return. {#t.interest_and_principal_go_in_different_sections_.57}
Interest and principal go into different parts of your ITR.

<a id="t.claimed_under.58"></a>

## Text — Claimed under {#t.claimed_under.58}
Claimed under

<a id="t.claimed_as.59"></a>

## Text — Claimed as {#t.claimed_as.59}
Claimed as

<a id="t.if_salaried_share_the_interest_and_principal_fig.60"></a>

## Text — If salaried, share the interest and principal figures with your emplo… {#t.if_salaried_share_the_interest_and_principal_fig.60}
If salaried, share the interest and principal figures with your employer for TDS.

<a id="t.documents_you_need.61"></a>

## Text — Documents you need {#t.documents_you_need.61}
Documents you need

<a id="t.interest_certificate.62"></a>

## Text — Interest certificate {#t.interest_certificate.62}
Interest certificate

<a id="t.from_the_lender_for_the_year_shows_interest_paym.63"></a>

## Text — From the lender for the year. Shows interest payment and principal re… {#t.from_the_lender_for_the_year_shows_interest_paym.63}
A statement given by the lender showing the yearly split between interest and principal.

<a id="t.possession_proof.64"></a>

## Text — Possession proof {#t.possession_proof.64}
Possession proof

<a id="t.needed_when_principal_repayment_and_pre_possessi.65"></a>

## Text — Needed when principal repayment and pre-possession interest payment s… {#t.needed_when_principal_repayment_and_pre_possessi.65}
Needed when principal repayment and pre-possession interest payment start.

<a id="t.anything_else_on_tax.66"></a>

## Text — Anything else on tax? {#t.anything_else_on_tax.66}
Anything else on tax?

<a id="t.there_are_a_few_extra_benefits_beyond_the_main_i.67"></a>

## Text — There are a few extra benefits beyond the main interest and principal… {#t.there_are_a_few_extra_benefits_beyond_the_main_i.67}
A few extra benefits beyond the interest and principal deductions.

<a id="t.claimed_in_the_year_you_pay_them_within_the_same.68"></a>

## Text — Claimed in the year you pay them, within the same ₹1.5 lakh principal… {#t.claimed_in_the_year_you_pay_them_within_the_same.68}
Claimed in the year you pay them, within the same ₹1.5 lakh principal repayment basket.

<a id="t.premium_can_sit_in_the_same_1_5_lakh_basket_if_y.69"></a>

## Text — Premium can sit in the same ₹1.5 lakh basket if you pay it yourself —… {#t.premium_can_sit_in_the_same_1_5_lakh_basket_if_y.69}
Premium counts inside the ₹1.5 lakh basket if paid directly, not if added to the EMI by the lender.

<a id="t.if_interest_creates_a_loss_only_up_to_2_lakh_can.70"></a>

## Text — If interest creates a loss, only up to ₹2 lakh can be set off against… {#t.if_interest_creates_a_loss_only_up_to_2_lakh_can.70}
Excess interest creating a loss up to ₹2 lakh offsets other income like salary in that year, with the rest carried forward.

<a id="t.you_usually_claim_principal_repayment_on_tax_onl.72"></a>

## Text — You usually claim principal repayment on tax only after a house is bu… {#t.you_usually_claim_principal_repayment_on_tax_onl.72}
Principal repayment claims begin only after a house is built on the plot and possession is taken.

<a id="t.under_the_old_tax_regime_only_and_only_if_this_w.73"></a>

## Text — Under the old tax regime only, and only if this was your first home, … {#t.under_the_old_tax_regime_only_and_only_if_this_w.73}
Under the old tax regime only, and only if this was your first home, older rules allowed extra interest for loans sanctioned in fixed windows.

<a id="t.your_home_loan_emi.75"></a>

## text {#t.your_home_loan_emi.75}
Your home loan EMI

<a id="t.has_tax_benefits.76"></a>

## text {#t.has_tax_benefits.76}
has tax benefits.

<a id="t.yes_your_limit_depends_on_who_lives_in_the_home_.77"></a>

## text {#t.yes_your_limit_depends_on_who_lives_in_the_home_.77}
Yes, your limit depends on who lives in the home and on the tax regime.

<a id="t.section_22.78"></a>

## text {#t.section_22.78}
Section 22

<a id="t.nbsp_ita_2025.79"></a>

## text {#t.nbsp_ita_2025.79}
&nbsp;ITA 2025

<a id="t.if_either_is_missing_only_30_000_a_year.80"></a>

## text {#t.if_either_is_missing_only_30_000_a_year.80}
If either is missing, only ₹30,000 a year.

<a id="t.yes_only_under_the_old_tax_regime.81"></a>

## text {#t.yes_only_under_the_old_tax_regime.81}
Yes, only under the old tax regime.

<a id="t.schedule_xv.82"></a>

## text {#t.schedule_xv.82}
Schedule XV

<a id="t.income_from_house_property.83"></a>

## text {#t.income_from_house_property.83}
Income from house property

<a id="t.a_deduction_from_total_income.84"></a>

## text {#t.a_deduction_from_total_income.84}
a deduction from total income

<a id="t.see_tax_claims_calculator.85"></a>

## text {#t.see_tax_claims_calculator.85}
See tax claims calculator

<a id="t.stamp_duty_and_registration_property_purchase_on.86"></a>

## text {#t.stamp_duty_and_registration_property_purchase_on.86}
Stamp duty and registration (property purchase only)

<a id="t.home_loan_protection_insurance.88"></a>

## text {#t.home_loan_protection_insurance.88}
Home loan protection insurance

<a id="t.house_property_loss_old_tax_regime.89"></a>

## text {#t.house_property_loss_old_tax_regime.89}
House property loss (old tax regime)

<a id="t.house_property_loss_new_tax_regime.91"></a>

## text {#t.house_property_loss_new_tax_regime.91}
House property loss (new tax regime)

<a id="t.interest_on_a_rented_home_can_still_reduce_renta.92"></a>

## text {#t.interest_on_a_rented_home_can_still_reduce_renta.92}
Interest on rented property reduces rental income, but such losses generally do not reduce salary or other income.

<a id="t.loan_for_a_plot_only_no_house_yet.93"></a>

## text {#t.loan_for_a_plot_only_no_house_yet.93}
Loan for a plot only (no house yet)

<a id="t.older_first_home_extras_only_if_your_loan_year_m.94"></a>

## text {#t.older_first_home_extras_only_if_your_loan_year_m.94}
Older first-home extras (only if your loan year matches)

<a id="t.section_130.95"></a>

## text {#t.section_130.95}
Section 130

<a id="t.covered_sanctions_from_1_april_2016_to_31_march_.96"></a>

## text {#t.covered_sanctions_from_1_april_2016_to_31_march_.96}
covered sanctions from 1 April 2016 to 31 March 2017 (up to ₹50,000 extra; loan up to ₹35 lakh; house up to ₹50 lakh).

<a id="t.section_131.97"></a>

## text {#t.section_131.97}
Section 131

<a id="t.covered_sanctions_from_1_april_2019_to_31_march_.98"></a>

## text {#t.covered_sanctions_from_1_april_2019_to_31_march_.98}
covered sanctions from 1 April 2019 to 31 March 2022 (up to ₹1.5 lakh extra; stamp value up to ₹45 lakh). This sits over and above the Section 22 interest limit. You cannot claim both. Not for loans sanctioned after 31 March 2022.
