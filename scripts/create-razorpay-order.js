#!/usr/bin/env node
/**
 * Create a Razorpay order and print the order_id for use on the test page.
 *
 * Get your keys from: https://dashboard.razorpay.com/app/keys
 * Use TEST keys for testing (rzp_test_...).
 *
 * Run (PowerShell):
 ...
 *
 * Run (bash/zsh):
 *   RAZORPAY_KEY_ID=rzp_test_xxxx RAZORPAY_KEY_SECRET=your_secret node scripts/create-razorpay-order.js
 */
var https = require('https');

var keyId = process.env.RAZORPAY_KEY_ID;
var keySecret = process.env.RAZORPAY_KEY_SECRET;
var amount = parseInt(process.env.RAZORPAY_AMOUNT || '100', 10); // paise, default 100 = ₹1
...
var req = https.request({
  ...
  res.on('end', function () {
    ...
    if (data.id) {
      console.log('Order created. Use this Order ID on the test page:\n');
      console.log(data.id);
      console.log('');
    } else {
