...
(function () {
  ...
  function ensureApplyFlowStyles() {
    ...
    style.textContent = [
      ...
      '#apply-button { position: fixed; left: 50%; bottom: 1.25rem; transform: translateX(-50%); z-index: 10000; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.15rem; padding: 0.6rem 1rem; min-height: 44px; width: min(320px, calc(100vw - 2rem)); font-family: Montserrat, system-ui, sans-serif; font-weight: 600; font-size: 0.95rem; background: var(--accent); color: #fff; border: none; border-radius: 100px; cursor: pointer; box-shadow: var(--shadow); transition: background 0.2s, transform 0.2s; } #apply-button:hover { background: var(--accent-hover); transform: translateY(-1px); } #apply-button:disabled { opacity: 0.6; cursor: not-allowed; transform: none; }',
      '#apply-button .apply-btn-line2 { font-size: 0.75rem; font-weight: 500; opacity: 0.95; }',
